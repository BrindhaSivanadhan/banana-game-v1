...
<head>
  <meta charset="utf-8"/>
  <title>Banana Bliss — Equation Game</title>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .digit-btn:hover:not(:disabled) {
      ...
      box-shadow: 0 10px 24px rgba(255, 225, 53, 0.5);
    }
    ...
    .btn.next:hover {
      ...
      box-shadow: 0 10px 24px rgba(255, 167, 38, 0.4);
    }
...
<body>
  ...
  <div class="card">
    <h1>🍌 Banana Equation Game</h1>
    <p class="subtitle">Solve the banana equation by guessing the missing value!</p>
    
    <div class="time">⏱️ Time: <span id="timer">30</span>s</div>
...
function onDigitClick(e){
  ...
  if (value === correct){
    document.getElementById('feedback').textContent = '✅ Correct! Preparing the advanced level...';
    setDigitsEnabled(false);
    stopTimer();
    ...
    }).then(r=>r.json()).catch(err=>console.error('Failed to save intermediate points', err)).finally(()=> {
      setTimeout(()=> window.location.href = 'advanced.php', 900);
    });
  } else {
